-- ClawdWire (ANY TOKEN) — paste this WHOLE file into the forked Dune query.
-- Do NOT paste this into 8180604; that one stays CLAWD-only as the safe fallback.
--
-- REQUIRES TWO TEXT PARAMETERS on the Dune query:
--   token_address   default 0x9f86db9fc6f7c9408e8fda3ff8ce4e78ac7a6b07
--   token_name      default CLAWD
...
--
-- Trade $ source: dex.trades PLUS transfer-fallback for Uniswap V4 / Clanker
-- hooked pools (Swap events can be 0/wrong). Re-paste after pulling this file.
--
...
WITH clawd AS (
    -- The ONLY difference from the CLAWD-only query. Everything downstream
    -- joins to this CTE, so parameterising it here makes the whole query
    -- work for any Base token.
...
-- Single 30d trade scan: short pulse windows + whale thresholds + 7d/24h tiers.
--
-- Clanker / Uniswap V4 hooked pools often emit Swap events with 0/wrong deltas
-- (beforeSwapReturnDelta / afterSwapReturnDelta), so dex.trades misses real
-- user fills. Primary path stays dex.trades; xfer_fills catches initiator
-- net token flow priced from ETH/WETH in the same tx, skipping hashes already
-- counted in dex.trades so we never double-count.
dex_trades AS (
    SELECT
        c.name    AS project,
        c.address AS address,
