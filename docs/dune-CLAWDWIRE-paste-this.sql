...
-- (V4 gap-fill is a separate MV doc — not inlined; stage limit)
-- Pulse $: 15m / 1h / 6h / 24h
...
-- Note: Uniswap V4 / Clanker hooked pools can miss from dex.trades. Do NOT inline
-- a transfer-fallback UNION here — this query is already at Dune's stage limit.
recent_trades AS (
    SELECT
        c.name    AS project,
        c.address AS address,
