...
const DUNE_REVALIDATE_SECONDS = 3600;
...
async function fetchDuneData({ fresh = false } = {}) {
  // Shared Next.js Data Cache across routes so lastUpdated matches everywhere.
  // Admin / force-refresh uses no-store so a new Dune run is visible immediately.
  const res = await fetch(
    `https://api.dune.com/api/v1/query/${DUNE_QUERY_ID}/results`,
    {
      ...
      ...(fresh
        ? { cache: "no-store" }
        : { next: { revalidate: DUNE_REVALIDATE_SECONDS, tags: ["dune-dashboard"] } }),
    }
  );
...
export async function getScoresLastUpdated() {
  const { lastUpdated } = await fetchDuneData();
  return lastUpdated;
}
...
export const getDashboardData = cache(async function getDashboardData() {
  return buildDashboardData({ fresh: false });
});

/** Bypass Next/Dune fetch cache — for admin copies after a new Dune run. */
export async function getDashboardDataFresh() {
  return buildDashboardData({ fresh: true });
}

async function buildDashboardData({ fresh = false } = {}) {
  const { rows: duneRows, lastUpdated } = await fetchDuneData({ fresh });
  const duneByAddress = {};
