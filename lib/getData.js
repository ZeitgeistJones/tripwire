...
export async function getDashboardSnapshotMeta() {
  const snap = await readDashboardSnapshot();
  if (!snap) return { lastUpdated: null, pricesUpdatedAt: null, builtAt: null };
  return {
    lastUpdated: snap.lastUpdated ?? null,
    pricesUpdatedAt: snap.pricesUpdatedAt ?? null,
    builtAt: snap.builtAt ?? null,
  };
}

async function fetchPriceLookups(addresses) {
  const priceLookup = await fetchCoinGeckoPrices(addresses);
  ...
  const dexLookup = missingAddresses.length
    ...
    : {};
  return { priceLookup, dexLookup };
}

/** Recompute price-dependent fields on an existing snapshot row (no Dune). */
function applyPricesToRow(row, priceLookup, dexLookup) {
  const addrKey = row?.Address?.toLowerCase?.();
  if (!addrKey) return row;
  ...
  const volumeGrowth = toFloat(row["Vol Grw %"]);
  const signal = cg ? getSignal(volumeGrowth, priceChange) : "No Data";

  let signalNote = null;
  if (signal === "Absorbed") {
    const wn = toFloat(row["Whale Net 7d"]);
    if (wn != null && Math.abs(wn) >= 250) signalNote = wn > 0 ? "whales in" : "whales out";
  }

  const whaleNet = toFloat(row["Whale Net 7d"]);
  const retailNet = toFloat(row["Retail Net 7d"]);
  const whaleNet24h = toFloat(row["Whale Net 24h"]);
  const retailNet24h = toFloat(row["Retail Net 24h"]);
 ...
 * Admin publishes updates: prices-only (default) or full Dune + prices.
 */
export const getDashboardData = cache(async function getDashboardData() {
  ...
  if (snap) {
    return {
      ...
      pricesUpdatedAt: snap.pricesUpdatedAt,
...
/**
 * Admin default refresh — CoinGecko/Dex only on existing Upstash rows.
 * No Dune Result Read (saves ~2 credits). Falls back to full rebuild if no snapshot.
 */
export async function refreshDashboardPrices() {
  const snap = await readDashboardSnapshot();
  if (!snap?.rows?.length) {
    return getDashboardDataFresh();
  }

  const allAddresses = snap.rows.map((r) => r.Address).filter(Boolean);
  const { priceLookup, dexLookup } = await fetchPriceLookups(allAddresses);
  const pricesUpdatedAt = new Date().toISOString();
  const rows = snap.rows.map((row) => applyPricesToRow(row, priceLookup, dexLookup));
  const data = {
    rows,
    lastUpdated: snap.lastUpdated ?? null,
    pricesUpdatedAt,
  };
  const written = await writeDashboardSnapshot(data);
  return { ...data, builtAt: written.builtAt };
}

/** Admin — rebuild from Dune + prices and publish (costs a Dune Result Read). */
export async function getDashboardDataFresh() {
  const data = await buildDashboardData();
  const snap = await writeDashboardSnapshot(data);
  return { ...data, builtAt: snap.builtAt, pricesUpdatedAt: snap.pricesUpdatedAt };
}
