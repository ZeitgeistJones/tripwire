...
/** Dune execution time from the published snapshot (same clock as the table). */
export async function getScoresLastUpdated() {
  const snap = await readDashboardSnapshot();
  if (snap?.lastUpdated) return snap.lastUpdated;
...
/**
 * Public + admin SSR: always serve the Upstash snapshot when fresh so every
 * surface (movers, dashboard, admin, banner) sees the same rows + lastUpdated.
 * Rebuilds from Dune when missing/stale and publishes for everyone.
 */
export const getDashboardData = cache(async function getDashboardData() {
  const snap = await readDashboardSnapshot();
  if (snap && snapshotIsFresh(snap)) {
    return { rows: snap.rows, lastUpdated: snap.lastUpdated };
  }
...
export async function getDashboardDataFresh() {
  ...
  // Must land in KV so public pages/banner see the exact same snapshot.
...
async function publishDashboardData() {
  ...
  try {
    ...
    // Still return the built payload so the request isn't empty; next successful
    // publish will realign.
