...
/**
 * Exclusive size bands for cohort pies / Who table.
 * Mega ⊂ old nested "whale"; here mega is split out so slices never overlap.
 *
 * Prefer true volume shares when Hump Vol % is present (after ClawdWire SQL
 * refresh). Until then, fall back to share of |net USD| — same exclusive
 * bands, labeled clearly in the UI.
 *
 * @param {"24h"|"7d"} window
 */
export function exclusiveCohortMix(row, window) {
  if (!row) return null;
  const is24 = window === "24h";
  const whaleVol = num(is24 ? row["Whale Vol % 24h"] : row["Whale Vol %"]);
  const humpVol = num(is24 ? row["Hump Vol % 24h"] : row["Hump Vol %"]);

  if (whaleVol != null && humpVol != null) {
    const mega = Math.max(0, Math.min(humpVol, whaleVol));
    const whaleOnly = Math.max(0, whaleVol - mega);
    const retail = Math.max(0, 100 - whaleVol);
    const sum = mega + whaleOnly + retail;
    if (sum <= 0) return null;
    return {
      mode: "volume",
      mega: (mega / sum) * 100,
      whaleOnly: (whaleOnly / sum) * 100,
      retail: (retail / sum) * 100,
    };
  }

  const whaleNet = num(is24 ? row["Whale Net 24h"] : row["Whale Net 7d"]) ?? 0;
  const humpNet = num(is24 ? row["Hump Net 24h"] : row["Hump Net 7d"]) ?? 0;
  const retailNet = num(is24 ? row["Retail Net 24h"] : row["Retail Net 7d"]) ?? 0;
  const mega = Math.abs(humpNet);
  const whaleOnly = Math.abs(whaleNet - humpNet);
  const retail = Math.abs(retailNet);
  const sum = mega + whaleOnly + retail;
  if (sum <= 0) return null;
  return {
    mode: "absNet",
...
/** Exclusive nets / counts: mega = hump fields; whale-only = whale − hump. */
export function exclusiveCohortMetrics(row) {
  if (!row) return null;
  const sub = (a, b) => {
    const x = num(a);
    const y = num(b);
  ...
  return {
    mega: {
      net24: num(row["Hump Net 24h"]),
      net7: num(row["Hump Net 7d"]),
      buyers24: num(row["Hump Buyers 24h"]),
      sellers24: num(row["Hump Sellers 24h"]),
      buyers7: num(row["Hump Buyers 7d"]),
      sellers7: num(row["Hump Sellers 7d"]),
    },
    whaleOnly: {
      net24: sub(row["Whale Net 24h"], row["Hump Net 24h"]),
      net7: sub(row["Whale Net 7d"], row["Hump Net 7d"]),
      buyers24: sub(row["Whale Buyers 24h"], row["Hump Buyers 24h"]),
      sellers24: sub(row["Whale Sellers 24h"], row["Hump Sellers 24h"]),
      buyers7: sub(row["Whale Buyers 7d"], row["Hump Buyers 7d"]),
      sellers7: sub(row["Whale Sellers 7d"], row["Hump Sellers 7d"]),
    },
    retail: {
      net24: num(row["Retail Net 24h"]),
      net7: num(row["Retail Net 7d"]),
      buyers24: null,
      sellers24: null,
      buyers7: null,
      sellers7: null,
...
export function megaFloorNote(humpMin) {
  const n = num(humpMin);
  if (n == null) return "";
  return n <= 1000 ? " (floor)" : "";
}
