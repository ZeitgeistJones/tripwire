...
/** Compact UTC hour label, e.g. "Aug 1 07:00Z". */
export function fmtUtcHour(v) {
  if (v == null || v === "") return "—";
  const d = parseDuneTs(v);
  if (!d) return String(v);
  const iso = d.toISOString();
  return `${MONTHS[d.getUTCMonth()]} ${d.getUTCDate()} ${iso.slice(11, 16)}Z`;
}

/** The two-digit UTC hour of a Dune timestamp, for lining up with the tape. */
...
export function parseHourlyTape(raw) {
  ...
  return String(raw)
    ...
    .filter(Boolean)
    .map((part) => {
      const m = part.match(/^(\d{1,2})\s*:\s*([+-]?[\d.]+)\s*k$/i);
      if (!m) return null;
      const usd = Number(m[2]) * 1000;
      return Number.isNaN(usd) ? null : { hour: m[1].padStart(2, "0"), usd };
    })
    .filter(Boolean);
}
...
export function freshness(iso) {
  ...
  if (mins <= 120) return { label: "recent", tone: "faint" };
