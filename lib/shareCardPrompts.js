 ...
 *  Cards stay outsider-friendly: dollars, counts, plain % only — no Accum / Div / Tripwire jargon.
 */
...
const PLAIN_NUMBER_RULES = [
  ...
  "- OUTSIDER-PLAIN ONLY: put dollars, wallet/trade counts, and simple shares of volume on the image.",
  "- Do NOT show jargon metrics even if you know them: Accum %, W/R Div / divergence bps, Opp/Mom/Sus, Qlty %, Risk %, Profile, Signal, Read, Buy/Sell ratio, Top10 %, Non-Trade New.",
  "- Labels must be human English (e.g. \"Large-wallet net flow\", \"Active wallets\", \"DEX volume\") — never raw field names.",
...
/** Identity + size only — no Tripwire score/signal jargon. */
...
export function buildWhaleCardPrompt(row, period = "7d", meta = {}) {
  ...
  return [
    ...
    "- A mega-trade = the top 1% of those sizes (floor $1,000).",
    ...
    "3. UP TO THREE CALLOUTS from DATA only — e.g. buyers vs sellers counts, mega-trade net, everyone-else net, large-trades share of volume, min size.",
    "4. Tiny definition of large vs mega trade + current min size.",
    "5. Snapshot time + disclaimer.",
    "Do not invent Accum %, divergence, scores, or signal labels.",
  ].join("\n");
}

/**
 * Plain status / health card — no Opp/Mom/Sus jargon on the image.
 */
export function buildHealthCardPrompt(row, period = "7d", meta = {}) {
  ...
  return [
    ...imageOutputRules(
      "THEME: clean status card — dark slate, big readable numbers, calm editorial type. Feels like a facts sheet, not a meme or a scoreboard of mystery scores."
    ),
    "",
    ...metaBlock(meta, `plain status at query snapshot · period ${period}`),
    "",
    "=== DATA (only these numbers may appear on the image) ===",
    JSON.stringify(stats, null, 2),
    "",
    ...
    "3. THREE plain facts from DATA (e.g. active wallets, DEX volume, large-wallet net flow).",
    "4. Optional one-liner that restates a fact — no bullish/bearish spin.",
    "5. Snapshot time + disclaimer.",
    "Do NOT put Opportunity / Momentum / Sustainability / Profile / Signal / Read on the image.",
  ].join("\n");
}
...
export function buildMoversCardPrompt(row, allRows, period = "7d", meta = {}) {
  ...
  return [
    ...
    "5. Snapshot time + disclaimer.",
    "If not highlighted, do NOT fake a top rank. No Accum % or score jargon.",
  ].join("\n");
}
...
export const SHARE_PROMPT_KINDS = [
  { value: "whale", label: "Whale", blurb: "Large-wallet flow — dollars & counts only" },
  { value: "health", label: "Health", blurb: "Plain status snapshot (no score jargon)" },
  { value: "activity", label: "Activity", blurb: "Volume / txs / wallets" },
  { value: "movers", label: "Movers", blurb: "Board highlight with simple peer numbers" },
];
