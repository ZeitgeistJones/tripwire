...
import { formatLocalDateTime } from "./snapshotTime";
...
function metaBlock(meta = {}, windowLabel) {
  const scoresAsOf = formatLocalDateTime(meta.scoresLastUpdated);
  return [
...
export function buildBuyersCardPrompt(row, period = "7d", meta = {}) {
  ...
  return [
    ...
    "- New buyers / sellers = wallets whose first buy or sell in the 90-day scan landed in this window — not lifetime first trade.",
    "",
    "=== DATA (only these numbers may appear on the image) ===",
    JSON.stringify(stats, null, 2),
    "",
    ...
    "3. UP TO THREE CALLOUTS — e.g. new-in-scan buyers vs sellers, buy share of volume, traders (30d only).",
...
/**
 * ClawdWire live-pulse share card — any tracked token's pulse row (+ optional dashboard row for mcap/price).
...
export function buildClawdWireCardPrompt(wireRow, dashRow = null, meta = {}) {
  ...
  const project = wireRow.Project || dashRow?.Project || "Token";
  const symbol = dashRow?.Symbol || wireRow.Symbol || project;
  ...
  const stats = {
    ...
    tradeShapeSignals: {
      note: "Research heuristics only — short-window round-trips, size clustering, and new-in-30d hold rates. Not proof of intent.",
      ...
      firstBuyerCohort30d: fmtInt(num(wireRow["1st Buyer Cohort 30d"])),
      surviveNote:
        "Survive % = share of wallets whose first buy in the last 30 days still hadn't sold by that horizon — not lifetime first buy, not balance still > 0.",
    },
  };

  const pulseAsOf =
    formatLocalDateTime(meta.clawdWireLastRunAt) ||
    formatLocalDateTime(meta.scoresLastUpdated);

  return [
    ...imageOutputRules(
      "THEME: deep-ocean ClawdWire pulse card — navy/black water, soft bioluminescent teal, subtle whale/ripple motif (tasteful). Live flow story, not a price chart."
    ),
    "",
    "=== TIMESTAMPS (must appear on the image — data time only) ===",
    pulseAsOf
      ...
      : "Data snapshot: unknown — say timing is approximate; do NOT invent a clock time",
    "Use ONLY this snapshot time on the card. Do not put 'generated now' / copy-time on the image.",
    "Numbers are from Tripwire ClawdWire (live Dune pulse for this token) joined with live mcap/price when present.",
    ...
    "- NEVER say \"first-time buyers ever\" / lifetime first buy. Survive and new-buyer stats are first buy inside the 30-day scan only.",
    ...
    "Survive = new-in-30d buyers still holding (no sell) after 1h/1d — not first buy ever.",
    "",
    "=== DATA (only these numbers may appear on the image) ===",
    JSON.stringify(stats, null, 2),
    "",
    ...
    "HIERARCHY:",
    `1. TITLE — ${symbol} · \"Live pulse\" or \"Large wallet flow\" (ClawdWire).`,
