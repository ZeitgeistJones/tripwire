...
export function buildWhaleCardPrompt(row, period = "7d", meta = {}) {
  ...
  return [
    ...
    "5. DATA SNAPSHOT TIMESTAMP (query time only) + DISCLAIMER.",
...
export function buildHealthCardPrompt(row, period = "7d", meta = {}) {
  ...
  return [
    ...
    "",
    ...metaBlock(meta, `scores + activity at query snapshot · period ${period}`),
    "",
    "=== DATA ===",
    JSON.stringify(stats, null, 2),
    "",
    ...
    "5. TINY FOOTER — market cap + period activity snapshot (wallets / vol) + data snapshot time + disclaimer.",
