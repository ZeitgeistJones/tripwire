...
export function buildAnalysisPrompt(d, history, meta = {}) {
  ...
  const payload = {
    ...
    whaleFlow7d: {
      ...
      megaWhaleMinTradeUsd: j(d["Hump Min $"]),
      whaleNetUsd: j(d["Whale Net 7d"]), accumPct: j(d["Accum %"]),
      whaleBuyers: j(d["Whale Buyers 7d"]), whaleSellers: j(d["Whale Sellers 7d"]),
      megaWhaleNetUsd: j(d["Hump Net 7d"]), megaWhaleBuyers: j(d["Hump Buyers 7d"]),
      megaWhaleSellers: j(d["Hump Sellers 7d"]),
  ...
  return [
    ...
    "- Whale trade: a DEX trade in the top 10% of sizes for this token over 30d (threshold given as whaleMinTradeUsd). Mega whale: top 1% with a $1k floor (megaWhaleMinTradeUsd — if it equals 1000, the token is on that floor). Retail net = all non-whale flow.",
    ...
    "3. WHALE STORY - read the whale/mega-whale/retail flows together. Who is doing what?",
