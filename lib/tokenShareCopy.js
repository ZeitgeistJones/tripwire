...
export function buildShareText(d, meta = {}) {
  ...
  return [
    ...
    "  Mega Whale Net: " + f(d["Hump Net 7d"], "usdSign") + " | Buyers: " + f(d["Hump Buyers 7d"], "int") + " / Sellers: " + f(d["Hump Sellers 7d"], "int"),
...
export function buildObjectiveText(d, meta = {}) {
  ...
  const megaLine =
    humpMinNum != null && !Number.isNaN(humpMinNum) && humpMinNum > 1000
      ? "  · Mega whale = top 1% of those sizes (definition includes a $1,000 floor). Every mega trade is also a whale.\n    → Current mega threshold: " + f(d["Hump Min $"], "usd") + "+ per trade."
  ...
  return [
    ...
    "Sources: Dune + CoinGecko/DexScreener.",
    "",
    "PRICE & SIZE",
    ...
    "  New buyers (first buy in 90d scan), last 7 days: " + f(d["1st Buyers 7d"], "int"),
    "  New sellers (first sell in 90d scan), last 7 days: " + f(d["1st Sellers 7d"], "int")
    ...
    "REFERENCE",
    "  Contract: " + (d.Address || "—"),
    "  Verify: basescan.org · dexscreener.com · tripwire-app.vercel.app",
  ].join("\n");
}
