...
const GLOSSARY_ROWS = [
  ...
  ["Buy USD · 15m / 1h / 6h / 24h (ClawdWire)", "DEX buy dollars in that window from Dune dex.trades, plus CLAWD-only V4/Clanker gap-fill from a materialized view when wired (estimate). Other tokens stay dex.trades-only"],
  ["Sell / Net USD · same windows (ClawdWire)", "Sell $ and buy−sell net from the same sources as Buy $"],
  ["Coverage % / DEX accounted / Unclassified $ · 15m / 1h / 6h / 24h (ClawdWire)", "Share of token transfer $ we could attribute to tagged DEX/V4 fills in that window. Unclassified includes remaining hooked swaps we still miss, plain transfers, and anything we couldn't venue-tag — an honesty bound, not a claim of total volume"],
