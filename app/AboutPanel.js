...
const GLOSSARY_ROWS = [
  ...
  ["Buy USD · 15m / 1h / 6h / 24h (ClawdWire)", "Buy $ from Dune dex.trades. CLAWD can optionally add V4/Clanker gap-fill from a separate materialized view (estimate) — off until you enable the UNION in the pulse SQL"],
  ["Sell / Net USD · same windows (ClawdWire)", "Sell $ and buy−sell net from the same sources as Buy $"],
  ["Coverage % / Accounted / Unclassified $ · 15m / 1h / 6h / 24h (ClawdWire)", "Honesty bound: share of transfer $ we could venue-tag in that window. Unclassified is the remainder (plain sends, still-missed hooks, etc.) — not a second volume total"],
