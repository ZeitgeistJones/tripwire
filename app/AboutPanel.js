...
const GLOSSARY_ROWS = [
  ...
  ["Buy / Sell / Net USD · 15m / 1h / 6h / 24h (ClawdWire)", "Tagged DEX trades on Base (Dune dex.trades). The main pulse signal — not every token transfer. CLAWD can optionally add a V4/Clanker gap-fill MV (off until enabled)"],
  ["Tagged share / Accounted / Other transfers · ClawdWire Data notes", "Advanced honesty check only: share of all token transfers that match a tagged DEX trade. Other transfers include fees, hops, and sends — not a second volume total, and not “trading we missed”"],
